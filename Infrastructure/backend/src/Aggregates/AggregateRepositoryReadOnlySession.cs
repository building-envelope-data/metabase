...
namespace Infrastructure.Aggregates
{
    public class AggregateRepositoryReadOnlySession : IAggregateRepositoryReadOnlySession
    {
        ...
        public async Task<Id> GenerateNewId(
        ...
        public async Task<Result<Timestamp, Errors>> FetchTimestamp<T>(
            Guid id,
            CancellationToken cancellationToken
            ) where T : class, IEventSourcedAggregate, new()
        {
              ...
              .Bind(streamState =>
                  {
                      ...
                      return Infrastructure.ValueObjects.Timestamp.From(streamState.LastTimestamp);
                  }
                  );
        }
        ...
        public async Task<Result<TimestampedId, Errors>> TimestampId<T>(
            Guid id,
            CancellationToken cancellationToken
            )
          where T : class, IEventSourcedAggregate, new()
        ...
        private async Task<IEnumerable<(Id, T?)>> LoadAllRaw<T>(
            IEnumerable<Id> ids,
            CancellationToken cancellationToken = default
            )
          where T : class, IEventSourcedAggregate, new()
        ...
        public Task<IEnumerable<Result<T, Errors>>> LoadAllX<T>(
            IEnumerable<TimestampedId> timestampedIds,
            Func<IAggregateRepositoryReadOnlySession, Type, IEnumerable<TimestampedId>, CancellationToken, Task<IEnumerable<Result<T, Errors>>>> loadAll,
            CancellationToken cancellationToken = default
            )
        {
            // TODO Avoid the unsafe cast of `t` to `TimestampedId`
            return LoadAllX<T>(
                timestampedIds.Select(x => ((Guid, DateTime))x),
                (session, aggregateType, idsAndTimestamps, cancellationToken) =>
                  loadAll(session, aggregateType, idsAndTimestamps.Select(t => (TimestampedId)t), cancellationToken),
                cancellationToken
                );
        }
        ...
        public Task<IEnumerable<IEnumerable<Result<T, Errors>>>> LoadAllBatched<T>(
            IEnumerable<(Timestamp, IEnumerable<Id>)> timestampsAndIds,
            CancellationToken cancellationToken = default
            )
          where T : class, IEventSourcedAggregate, new()
        {
            return LoadAllBatched<T>(
                timestampsAndIds.Select(((Timestamp timestamp, IEnumerable<Id> ids) t) =>
        ...
        public Task<IEnumerable<IEnumerable<Result<T, Errors>>>> LoadAllThatExistedBatched<T>(
            IEnumerable<(Timestamp, IEnumerable<Id>)> timestampsAndPossibleIds,
            CancellationToken cancellationToken = default
            )
          where T : class, IEventSourcedAggregate, new()
        {
            return LoadAllThatExistedBatched<T>(
                timestampsAndPossibleIds.Select(((Timestamp timestamp, IEnumerable<Id> possibleIds) t) =>
                  ((DateTime)t.timestamp,
        ...
        private async Task<IEnumerable<Id>> QueryModelIds<TCreatedEvent>(
            CancellationToken cancellationToken
            )
          where TCreatedEvent : ICreatedEvent
        {
        ...
        public
          ...
          GetForwardManyToManyAssociationsOfModels<TModel, TAssociationModel, TAggregate, TAssociationAggregate, TAssociationAddedEvent>(
            IEnumerable<TimestampedId> timestampedModelIds,
            CancellationToken cancellationToken
            )
          where TAggregate : class, IEventSourcedAggregate, IConvertible<TModel>, new()
        ...
        private async
          ...
          GetAssociationsOrAssociatesOfModels<TModel, TAssociateModel, TAggregate, TAssociateAggregate>(
            IEnumerable<TimestampedId> timestampedModelIds,
            Func<IEnumerable<Id>, CancellationToken, Task<IEnumerable<(Id modelId, Id associateId)>>> queryAssociateIds,
            CancellationToken cancellationToken
            )
          where TAggregate : class, IEventSourcedAggregate, IConvertible<TModel>, new()
          where TAssociateAggregate : class, IEventSourcedAggregate, IConvertible<TAssociateModel>, new()
        ...
        private async
          Task<IEnumerable<(Id modelId, Id associationId)>>
          QueryForwardAssociationIds<TAssociationAddedEvent>(
            IEnumerable<Id> modelIds,
            CancellationToken cancellationToken
            )
          where TAssociationAddedEvent : Events.IAssociationAddedEvent
        {
            ...
            return (await QueryEvents<TAssociationAddedEvent>()
                ...
                .ToListAsync(cancellationToken)
                .ConfigureAwait(false))
              .Select(a => ((Id)a.ModelId, (Id)a.AssociationId));
        }
        ...
        private async
          ...
          GetAssociatesOfModels<TModel, TAssociationModel, TAssociateModel, TAggregate, TAssociationAggregate, TAssociateAggregate>(
            IEnumerable<TimestampedId> timestampedIds,
            Func<TAssociationModel, Id> getAssociateId,
            Func<IEnumerable<TimestampedId>, CancellationToken, Task<IEnumerable<Result<IEnumerable<Result<TAssociationModel, Errors>>, Errors>>>> getAssociations,
            CancellationToken cancellationToken
            )
