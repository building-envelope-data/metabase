...
public sealed record HygrothermalData(
    string Id,
    Guid Uuid,
...
)
{
    ...
    [NodeResolver]
    public static Task<HygrothermalData?> GetAsync(
        string id,
    ...
    {
        return DataX.Data.FetchNodeAsync<HygrothermalData>(
            id,
            (database, uuid, locale) => dataQueries.GetHygrothermalDataAsync(
                database,
                uuid,
                locale,
                resolverContext,
