...
namespace Icon.Configuration
{
    public sealed class EventStore
    {
        ...
        private static void ProjectAggregatesInline(Marten.StoreOptions options)
        {
            ...
            // `ls -1 src/Aggregates/ | grep -E ".*Aggregate.cs$" | grep -v -E "^MethodDeveloperAggregate.cs$" | grep -v -E "^I((Many|One)ToMany)?AssociationAggregate.cs$" | sed -e "s/^\(.*\).cs\$/options.Events.InlineProjections.AggregateStreamsWith<Aggregates.\1>();/"`
            ...
            options.Events.InlineProjections.AggregateStreamsWith<Aggregates.ComponentOpticalDataAggregate>();
        ...
        {
            // Originally generated from the command line with the command
            // `ls -1 src/Events/ | grep "Created\|Added\|Deleted\|Removed" | grep -v -E "^I?(Created|Deleted)Event.cs$" | grep -v -E "^I?Association(Added|Removed)Event.cs$" | grep -v -E "^MethodDeveloperAdded.cs$" | sed -e "s/^\(.*\).cs\$/options.Events.AddEventType(typeof(Events.\1));/"`
            ...
            options.Events.AddEventType(typeof(Events.ComponentOpticalDataAdded));
            options.Events.AddEventType(typeof(Events.ComponentOpticalDataRemoved));
