...
const negateIfNecessary = (
	negator: Negator,
	proposition: LifeCycleDataPropositionInput,
): LifeCycleDataPropositionInput => {
	switch (negator) {
		case Negator.Is:
			return proposition;
...
const conjunct = (
	propositions: LifeCycleDataPropositionInput[],
): LifeCycleDataPropositionInput => {
...
function Page() {
	...
	const [data, setData] = useState<LifeCycleDataPartialFragment[]>([]);
	...
	const allLifeCycleDataQuery = useQuery(AllLifeCycleDataDocument, {
		skip: true,
		errorPolicy: "all",
	});
	...
	const onFinish = ({
		...
		const filter = async () => {
			try {
				...
				const propositions: LifeCycleDataPropositionInput[] = [];
				if (componentIds) {
				...
				const { error, data } = await allLifeCycleDataQuery.refetch(
				...
				const nestedData =
					data?.databases?.edges?.map(
						(edge) =>
							edge?.node?.allLifeCycleData?.edges?.map((e) => e.node) || [],
					) || [];
				const flatData = ([] as LifeCycleDataPartialFragment[]).concat(
					...nestedData,
				);
				setData(flatData);
			} catch (error) {
